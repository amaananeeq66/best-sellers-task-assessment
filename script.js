        ...
        return Array.from(
            { length: 5 },
            () =>
            `<svg aria-hidden="true" viewBox="0 0 20 20" class="h-[12px] w-[12px] fill-[#111]"><path d="M10 15.27 16.18 19l-1.64-7.03L20 7.24l-7.19-.62L10 0 7.19 6.62 0 7.24l5.46 4.73L3.82 19z"></path></svg>`,
        ).join("");
        }
        ...
        const badgeClass = isSavings
            ...
            : "bg-white text-[#111] border-black";
        return `<span class="inline-flex items-center rounded-full border px-[8px] font-condensed text-[10px] py-[4px] transition transform hover:scale-105 hover:shadow-md ${badgeClass}">${badge}</span>`;
        }
        ...
        return `
                                                    <li class="${desktop ? "shrink-0" : "w-full"}" ${desktop ? `style="width:${desktopCardWidth}px"` : ""}>
                                                        <article class="group hover:shadow-lg transition-shadow duration-300">
        ...
        desktopProducts.innerHTML = products
            .map((p) => createCardMarkup(p, true))
            .join("");
        mobilePrimaryProducts.innerHTML = products
            .slice(0, 4)
            .map((p) => createCardMarkup(p, false))
            .join("");
        ...
        setMobileExpanded(showMoreBtn.textContent !== "Show More"),
        );
        ...
        const thumbLeft =
            (desktopRail.scrollLeft / (contentWidth - visibleWidth)) * maxThumbTravel;
        ...
        let thumbLeft = Math.min(
            Math.max(e.clientX - trackRect.left - dragStartX, 0),
            maxTravel,
        );
        const ratio = thumbLeft / maxTravel;
        desktopRail.scrollLeft =
            ratio * (desktopRail.scrollWidth - desktopRail.clientWidth);
        });

        ["pointerup", "pointercancel"].forEach((ev) =>
        desktopThumb.addEventListener(ev, () => {
            isDraggingThumb = false;
            desktopThumb.releasePointerCapture();
        }),
        );
        ...
        desktopRail.addEventListener("pointerdown", (e) => {
        isDraggingRail = true;
        startX = e.pageX - desktopRail.offsetLeft;
        scrollLeft = desktopRail.scrollLeft;
        desktopRail.classList.add("cursor-grabbing");
        cancelAnimationFrame(momentumID);
        });

        desktopRail.addEventListener("pointerleave", () => (isDraggingRail = false));
        desktopRail.addEventListener("pointerup", () => (isDraggingRail = false));

        desktopRail.addEventListener("pointermove", (e) => {
        if (!isDraggingRail) return;
        const x = e.pageX - desktopRail.offsetLeft;
        const walk = (x - startX) * 2;
        velocity = walk;
        desktopRail.scrollLeft = scrollLeft - walk;
        });

        function momentum() {
        desktopRail.scrollLeft -= velocity;
        velocity *= 0.95;
        ...
        desktopRail.addEventListener("pointerup", () => momentum());
        desktopRail.addEventListener("pointerleave", () => momentum());
